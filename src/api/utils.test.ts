...
import { fetchJson, buildUrl, buildPaginationParams, withRetry } from './utils';
...
describe('fetchJson with retry', () => {
  beforeEach(() => {
    mockFetch.mockReset();
  });

  it('retries on network error when retry is enabled', async () => {
    mockFetch
      .mockRejectedValueOnce(new Error('Failed to fetch'))
    ...
    // Use very short delays for testing
    const result = await fetchJson('/test', 'test', { retry: true, maxRetries: 2, baseDelay: 1 });

    expect(result).toEqual({ data: 'success' });
    expect(mockFetch).toHaveBeenCalledTimes(2);
  });

  it('does not retry when retry is not enabled', async () => {
    mockFetch.mockRejectedValueOnce(new Error('Failed to fetch'));

    await expect(fetchJson('/test', 'test')).rejects.toThrow('Failed to fetch');
