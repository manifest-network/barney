...
export const AI_TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    function: {
      name: 'update_app',
      description: 'Update an app with a new manifest file, a new Docker image, or a new service stack definition. On pr240, retry an unresolved update with only app_name to recover its original key and exact payload. After reload, Barney checks provider history for matching bytes, or can verify the original attached file merged with saved defaults by its payload fingerprint. If no match exists, exact reviewed bytes are required; never regenerate passwords or replace a pending command. A settled-command notice prevents stale retries from creating another command; use new_command only for deliberate new work after that notice. Legacy v0.13 cannot deduplicate retries: observe app_status and app_releases before deliberately submitting another update. Never automatically stop/redeploy as recovery. If exact bytes are permanently lost and no recovery route remains, explain that a separately confirmed stop_app ends the deployment; it does not recover the update, and Fred may execute the pending command until the lease closes.',
      parameters: {
        type: 'object',
        properties: {
          app_name: {
