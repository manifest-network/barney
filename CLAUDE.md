...
`src/registry/appRegistry.ts` — wallet-scoped app cache with optional localStorage persistence and an in-memory fallback. `discoverAppsFromChain` imports missing active/pending leases with stable lease-derived names and unconfirmed readiness, deriving status through `deriveAppStatus`. Sidebar reconciliation and `list_apps` both discover inventory after clearing browser storage. `src/api/appDiscovery.ts` resolves missing catalog metadata; `useAppRecovery` independently reads authenticated provider status/connections. Complete paginated tenant reads prevent a truncated page from marking leases absent. Discovery/deploy races merge by lease UUID; stale provider reads cannot overwrite a newer registry snapshot.
