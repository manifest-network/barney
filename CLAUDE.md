...
URL shaping distinguishes missing port records from records emptied by SDK validation. An emptied record cannot establish HTTP routing from an FQDN; independent status endpoints remain usable. Instance HTTP FQDNs can route through Traefik without a published host port, while instance TCP mappings retain the reported Docker host. `connectionPatch` preserves stored URLs and connection details when a read provides no usable endpoint. A newly observed URL takes precedence, including a portless HTTPS FQDN after update; when no new connection accompanies it, the saved service inventory and DNS targets remain available. `refreshAppConnection` shares refresh logic between `app_status` and background recovery. Status cards label cached service details separately from a refreshed primary endpoint and hide access controls for terminal leases.
