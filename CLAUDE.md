...
five-minute intervals (eight total). Failed status reads do not create readiness staleness. The readiness flag
...
`useAppRecovery` reads the registry on its own tick. During status queries and discovery, a rejected
status fetch or token mint leaves the flag unchanged. An arrived response with omitted, empty or unmodelled status still schedules follow-up. Fresh connection
