...
⚠️ **Never wrap `deployManifest` in `signing.withSign`.** It mints its own ADR-036 lease-data token through the same non-reentrant signing mutex, so wrapping it deadlocks (deployManifest → `providerAuth.leaseDataToken` → same mutex → circular wait). Chain-TX serialization comes from `CosmosClientManager.withBroadcastLock` plus the mutex-wrapped `signArbitrary` instead — `withSign` is for the raw `cosmosTx` tools (`fund_credits`, `cosmos_tx`). `stop_app` now delegates to the SDK's `stopApp` and is likewise **never** wrapped in `withSign` (it holds the broadcast lock internally; wrapping it would deadlock).
