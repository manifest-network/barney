...
`useAppRecovery` separately recovers provider readiness and connection details one app at a time while the user is idle. Neither chat listing nor chain reconciliation waits for provider reads or signatures. Each recovery attempt has an aggregate deadline, uses a distinct one-time credential per provider request, and yields to foreground work. Cancelled rounds retain their attempt budget and yield their place to other eligible apps. Unsettled or incomplete responses retry with per-app backoff, up to four attempts per unchanged app snapshot. Failed verdicts and confirmed workloads with no published endpoint retire from automatic recovery; `app_status` remains the explicit refresh path. An active chain lease alone never confirms a recovered workload is running.
