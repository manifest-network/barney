...
`app_status` always returns the `app` display card for a resolved registry entry. `MessageBubble` renders `AppCard` with the registry-derived status, the independent provider progress reading, endpoint, and stack service connections; domain controls and full tool details are collapsed secondary actions. Status, primary endpoint, and service inventory have independent availability flags. `refreshAppConnection` preserves an established URL, service inventory, and DNS targets when the connection read fails. Terminal endpoints are hidden, while Stop eligibility follows the chain verdict separately from the workload verdict. Flat deployment ports remain visible without inventing per-service assignments. `appCardConnection.ts` validates permissively stored fields before rendering; cards subscribe only to the store fields they use.

Sidebar selection calls `requestAppStatus` directly: it bypasses the tool cache for a fresh read and makes no Morpheus request. The action records the actual tool call/result for subsequent chat context, preserves pending attachments, and guards completion by abort controller and wallet epoch. Model-initiated status results set `continueConversation` so a restart/update workflow can resume after rechecking readiness. A batch continues only if every display card opts in; transaction confirmations always pause the turn. Cards remain transient and are rebuilt by a new status request after history reload.
