...
`app_status` always returns the `app` display card for a resolved registry entry. `MessageBubble` renders `AppCard` with the registry-derived status, the independent provider progress reading, endpoint, and stack service connections; domain controls and full tool details are collapsed secondary actions. Lease observations, workload readings, primary endpoints, and service inventories have independent availability flags. An ACTIVE chain reading keeps the registry-derived badge visible even if workload status is unavailable; an unavailable lease reading is labeled separately. `refreshAppConnection` preserves an established URL, service inventory, and DNS targets when the connection read fails; a differing provider-reported endpoint is offered separately for copying, while a matching status endpoint corroborates the saved URL. Lifecycle operations independently invalidate saved connection metadata with `connectionStale`; DNS target resolution ignores that inventory until a fresh read clears the flag. Target changes reset cached DNS verdicts, and background recovery retries invalidated connections even for confirmed apps. Terminal workload endpoints are hidden. Attached-domain controls remain available for PENDING and ACTIVE chain leases, including when only the provider reports a terminal workload; Stop eligibility also follows the chain verdict separately from the workload verdict. Saved DNS and qualified URLs remain intact, while legacy bare IPv4 URLs get their assigned ports consistently in status and lifecycle results. Flat deployment ports remain visible without inventing per-service assignments. `appCardConnection.ts` normalizes live and legacy port mappings, attributes flat data only when the complete lease service set identifies one service (deduplicating repeated names), and retains provider-named services when chain items are only partly named. Ambiguous hoisted fields are omitted when reported names differ from the lease or manifest. Every app-card producer uses the connection normalizer. Returned empty inventories replace stored ports independently of URL availability; port zero is unassigned, and explicit empty service records show "No published ports". Missing or malformed records remain unavailable. Cached service details stay in a collapsed disclosure; chain-only reads label saved access data without suggesting a failed refresh. App cards subscribe only to their own domain entry.

Sidebar selection calls `requestAppStatus` directly: it bypasses the tool cache for a fresh read and works independently of Morpheus relay health. It retains the SDK query deadlines and uses `withAbort` solely to stop waiting on cancellation, so slow provider reads can still produce the executor's chain-only overview. The action records the actual tool call/result for subsequent chat context, preserves pending attachments, and guards completion by abort controller and wallet epoch. Model-initiated status results set `continueConversation` so a restart/update workflow can resume after rechecking readiness. A batch continues only if every display card opts in; transaction confirmations always pause the turn. Confirmed dispatch clears query caches before mutation, including when the operation later fails or has an ambiguous outcome. An explicit `unknown` provider reading is displayed and records unconfirmed readiness when no previous confirmation exists. Absent readings supply no new provision-state observation. Sidebar selection retains keyboard focus using `aria-disabled` during its own status read. Other active requests cause feedback only when the user tries to select an app or stop one. Sidebar busy feedback is cleared when its blocker ends and does not return on a later unrelated request or confirmation. Accepted selections immediately close the mobile sidebar once the chat placeholder exists, exposing progress, cancellation, and errors in chat; start refusals stay visible in the sidebar. Historical Stop cards explain inactive leases and unavailable wallet authorization. Cancellation of model tool execution finalizes every advertised tool reply, including unstarted calls and collected confirmations; the next chat request retains a valid tool-call group. Displayed provider progress is bounded and sanitized independently of raw-value classification. Only app cards expose supplemental raw tool details; log and custom-domain cards do not duplicate their contents in a second JSON disclosure. Cards remain transient and are rebuilt by a new status request after history reload.
