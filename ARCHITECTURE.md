...
`app_status` always returns the `app` display card for a resolved registry entry. `MessageBubble` renders `AppCard` with the registry-derived status, endpoint, and stack service connections; domain controls and full tool details are collapsed secondary actions. Status, primary endpoint, and service inventory have independent availability flags. A partial refresh preserves cached service inventory and DNS targets, while terminal leases show an inactive endpoint without access controls. `refreshAppConnection` shares endpoint refresh logic with background recovery; `appCardConnection.ts` validates permissively stored connection fields before rendering. Status results set `continueConversation` so a restart/update workflow can resume after rechecking readiness; transaction confirmations still pause the turn. Cards remain transient and are rebuilt by a new status request after history reload.
