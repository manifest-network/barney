...
it('fails the configured postinstall when a required patch cannot apply outside CI', async () => {
  ...
  try {
    const installedPackage = join(fixture, 'node_modules/@cosmos-kit/web3auth');
    await mkdir(join(installedPackage, 'esm/extension'), { recursive: true });
    await mkdir(join(fixture, 'patches'));
    await writeFile(join(fixture, 'package.json'), JSON.stringify({
      name: 'barney-patch-failure-fixture',
      private: true,
      dependencies: { '@cosmos-kit/web3auth': 'npm:@manifest-network/web3auth@2.16.6-ll.1' },
    }));
    await writeFile(join(installedPackage, 'package.json'), JSON.stringify({ name: '@manifest-network/web3auth', version: '2.16.6-ll.1' }));
    ...
    await writeFile(join(installedPackage, 'esm/extension/signer.js'), 'export class Web3AuthSigner {}\n');
    const patch = '@cosmos-kit+web3auth+2.16.6-ll.1.patch';
    await copyFile(join(root, 'patches', patch), join(fixture, 'patches', patch));
    ...
    const child = spawnSync(process.execPath, [patchCli, ...args], {
      ...
      env: { PATH: process.env.PATH, TMPDIR: fixture, NODE_ENV: 'development', CI: 'false' },
      encoding: 'utf8',
      timeout: 5_000,
      killSignal: 'SIGKILL',
      maxBuffer: 128 * 1024,
    ...
    expect(output).toContain('Failed to apply patch for package @cosmos-kit/web3auth');
