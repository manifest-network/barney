...
Select an app in the sidebar to check its status and deployment endpoint. The overview shows service endpoints for stacks and keeps custom domains behind **Set custom domain** or **Manage custom domains**. Expand **Details** for size, image, and creation time. If current status cannot be confirmed, the card shows **Status unavailable** and the recorded status. Access details that could not be refreshed are labeled **Last known endpoint** or **Last known service details**; missing endpoints are shown as **Endpoint unavailable**. Closed deployments show an inactive endpoint. **Stop** remains available for failed workloads with an active lease.
