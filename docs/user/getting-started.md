...
Select an app in the sidebar to refresh its status and deployment endpoint directly, even when AI chat is unavailable. The overview shows service endpoints for stacks and keeps custom domains behind **Set custom domain** or **Manage custom domains**. Expand **Details** for size, image, and creation time. Provider progress, such as **restarting**, appears alongside the recorded status. Lease status is shown separately from workload status. If the provider cannot answer, the card keeps the recorded status and shows **Workload status unavailable**; **Status unavailable** means the lease query also could not be confirmed. Cached service details can be expanded under **Last known service details**, or **Saved service details** when provider access was not queried. The endpoint is likewise labeled as saved or last known; missing endpoints are shown as **Endpoint unavailable**. If the provider reports a different endpoint during a connection outage, it appears separately with its own copy button. Internal services show **No published ports**. Older stack data can show **Deployment ports** with a note that per-service details are unavailable. Closed deployments show an inactive endpoint. **Stop** remains available for failed workloads with an active lease. An older Stop card explains if that lease has already closed. While another request or confirmation is active, clicking an app explains how to finish that action first. The notice clears when the action ends.
