...
The model can invoke only registered tools. All mutations use typed high-level Manifest SDK operations; neither model dispatch nor confirmed dispatch accepts arbitrary chain transactions. The shared `transactionPlans.ts` schemas validate semantic action data during planning and again at confirmation. Unknown raw fields and fee overrides are rejected. Credit funding accepts only a positive PWR amount representable in six decimal places and safe integer base units, derives the coin string locally, and funds the signer's own account.

`useManifestMCP` configures a per-transaction gas ceiling of 50,000,000. The SDK rejects an inflated gas estimate before signing; the configured gas price turns this into a fee ceiling. Confirmation cards show that maximum (summed for bulk operations, including the possible second domain transaction per deploy). Actual fees can be lower. Restart/update are provider operations with no chain fee. Cancellation reaches the SDK; a cancellation after submission cannot undo a broadcast, so uncertain credit funding tells the user to check their balance before retrying.

**Web3Auth approval policy.** `src/main.tsx` deliberately sets `promptSign: async () => true`. Web3Auth invokes it for both direct and Amino signatures, so it does not provide a second human approval dialog. Human approval for chat mutations is the confirmation card, consumed once by `confirmAction`, with wallet/chain/client/signer-generation checks repeated by confirmed dispatch and before each SDK mutation. `transactionConsent.test.tsx` exercises real model dispatch, rendered Confirm/Cancel, and an automatically approving signing seam. Removed/invented tools, malformed actions, cancelled requests, and replayed confirmations cannot invoke a mutation.

Two existing non-chat flows use the signer: ADR-036 provider/relay proofs authenticate off-chain requests, and first-connect account setup uses a fixed self-funding SDK operation when a faucet is configured. Account setup does not use a chat confirmation card; its amount and recipient come from application policy, never model arguments. Both initial and retry funding calls receive the setup abort signal. See [the complete transaction inventory](transaction-boundary.md).
