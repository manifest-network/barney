...
- `transactionBoundary.test.ts` rejects low-level transaction imports, signing/broadcast access, and locally declared raw APIs, including calls inside runtime class `extends` expressions. Known raw API names are deliberately forbidden as runtime object/class members or function bindings, even in handler maps, because those shapes can implement a signer or broadcaster. Type annotations, interface inheritance, class `implements` clauses, and narrowing checks remain allowed.
